...
void GetImagesROS::aprilTagCallBack(const DC_VSLAM::AprilTagDetectionArray::ConstPtr& msg)
{
    ...
    if ( ATFound )
        return;
    bool closeToAT = sqrt(pow(tra.x,2) + pow(tra.z,2)) < 1;
    bool yawCorr = abs(pitch) < 0.025;
    bool xCorr = abs(tra.x) < 0.03;
    bool yCorr = abs(tra.z) < 0.33;
    if (!(closeToAT && yawCorr && xCorr && yCorr))
        return;
    Eigen::Matrix4d startPose;
    startPose << 1,0,0,-2.82,0,1,0,0,0,0,1,4.32,0,0,0,1;
    Eigen::Matrix4d tagposel = startPose * voSLAM->mZedCamera->cameraPose.pose * T_c1_AT * Tc2_tag;
    std::cout << "AprilTag Detected!\n" << tagposel << "\n" << std::endl;
    std::cout << "CAMERA POSE!\n" << startPose* voSLAM->mZedCamera->cameraPose.pose << std::endl;
    ...
    voSLAM->map->LCPose = startPose.inverse() * tagPose * Tc2_tag.inverse() * T_c1_AT.inverse();
    voSLAM->map->aprilTagDetected = true;
    std::cout << "CAMERA POSE AFTER!\n" << voSLAM->map->LCPose << std::endl;
}
